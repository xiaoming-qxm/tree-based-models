...
int main(void)
{
  ...
  best_feature_idx = ent.BestInfoGain(data_idx, feat_idx, data, labels, num_classes, num_feature);
  cout << "Use info gain, best feature index: " << best_feature_idx << endl;
  cout << "Impurity value:  " << ent.get_impurity() << endl;

  best_feature_idx = ent.BestInfoGainRatio(data_idx, feat_idx, data, labels, num_classes, num_feature);
